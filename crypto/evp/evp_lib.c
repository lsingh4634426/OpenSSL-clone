...
int EVP_CIPHER_CTX_set_algor_params(EVP_CIPHER_CTX *ctx, const X509_ALGOR *alg)
{
    ...
    if ((derl = i2d_ASN1_TYPE(alg->parameter, &der)) >= 0) {
         ...
         */
        *p++ =
            OSSL_PARAM_construct_octet_string(k_old, der, (size_t)derl);
        *p++ =
            OSSL_PARAM_construct_octet_string(k_new, der, (size_t)derl);
...
int EVP_CIPHER_CTX_get_algor_params(EVP_CIPHER_CTX *ctx, X509_ALGOR *alg)
{
     ...
     */
    *p++ = OSSL_PARAM_construct_octet_string(k_old, NULL, 0);
    *p++ = OSSL_PARAM_construct_octet_string(k_new, NULL, 0);
    *p = OSSL_PARAM_construct_end();
