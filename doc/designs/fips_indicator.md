...
- If a module only supports approved services in an approved manner an explicit indicator can be used (e.g. successful completion of a service is an indicator).
...
If we had different kinds of compliance requirements (something other than FIPS) either a seperate getter could be added or the getter could return a int type instead of a 0 or 1..
...
- KBKDF CMAC
- GMAC
Should only allow AES. We currently just check the mode.
