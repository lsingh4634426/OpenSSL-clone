...
for consistency with the given Extended Key Usage (EKU) or purpose.
If an EKU is given it overrides any purpose requirements for the entire chain.
Then each certificate must either not have an EKU extension,
or the EKU extension must include the given required EKU
or the special 'wildcard' EKU B<anyExtendedKeyUsage>.
