...
Certificate validation can be done relatively to a given Extended Key Usage
(EKU) or I<purpose> specifying the intended use of the target certificate,
such as SSL server, or (by default) for any EKU and any purpose.
...
The second step is to check the extensions of every certificate
for consistency with any given Extended Key Usage (EKU) or purpose.
If an EKU is given it overrides any purpose requirements for the entire chain.
Then the target or "leaf" certificate (which may be an end-entity
certificate or CA certificate) must either not have an EKU extension,
or the EKU extension must include the specified EKU or the 'wildcard' EKU
B<anyExtendedKeyUsage>, or the specified EKU is B<anyExtendedKeyUsage>.
If no B<-purpose> or B<-eku> option is given then no such checks are done
...
The required Extented Key Usage of the leaf certificate in the chain.
This overrides the purpose checks for all certificates in the chain.
The EKU I<arg> can be an object name or an OID in numeric form.

For details see the L</Certification Path Validation> section above.

=item B<-eku_included>

Modify the B<-eku> check such that an EKU cerificate extension must be present
and must include the specified EKU.
