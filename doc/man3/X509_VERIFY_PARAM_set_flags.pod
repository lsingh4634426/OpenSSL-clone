...
X509_VERIFY_PARAM_set_eku, X509_VERIFY_PARAM_set_purpose,
...
=head1 SYNOPSIS
 ...
 int X509_VERIFY_PARAM_set_eku(X509_VERIFY_PARAM *param, int nid);
...
X509_VERIFY_PARAM_set_eku() sets the given I<nid>
as the expected Extended Key Usage (EKU) in I<param>.
Using B<NID_undef> resets the requirement.
Otherwise the NID must correspond to an EKU OID, for example B<NID_server_auth>.
It may have been obtained by parsing a text string using L<OBJ_txt2nid(3)>.
With SSL, likely only B<NID_client_auth> or B<NID_server_auth> will make sense.
If provided, the NID overrides any purpose requirements for the entire chain
and determines the acceptable EKU for the target of "leaf" certificate
in the chain, which may be an end-entity certificate or CA certificate.
This certificate must either not have an EKU extension,
or the EKU extension must include the desired EKU or B<anyExtendedKeyUsage>,
or I<nid> equals B<anyExtendedKeyUsage>.
If the B<X509_V_FLAG_EKU_INCLUDED> verification flag is set the EKU extension
must be present in the certificate and must include the specified EKU I<nid>.
...
X509_VERIFY_PARAM_set_flags(), X509_VERIFY_PARAM_clear_flags(),
X509_VERIFY_PARAM_set_inh_flags(), X509_VERIFY_PARAM_set_eku(),
...
The B<X509_V_FLAG_EKU_INCLUDED> flag modifies any enabled EKU check such that
an EKU certificate extension must be present and must include the specified EKU.
...
The X509_VERIFY_PARAM_set_eku(),
X509_VERIFY_PARAM_get0_host(), X509_VERIFY_PARAM_get0_email(),
