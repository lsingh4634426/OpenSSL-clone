...
and determines the acceptable EKU for all certificates in the chain,
from the peer certificate up to and including the trust anchor certificate.
Each of these certificates must either not have an EKU extension,
